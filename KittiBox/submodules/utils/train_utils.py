...
def add_rectangles(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25,
    ...
    draw_candidate = [] # this is the group
    for rect_set, color in pairs:
        for rect in rect_set:
            if rect.confidence > min_conf:
                draw_candidate.append(rect)
                #_draw_rect(draw, rect, color)
                # print("Drew a rec here !!!!")

    # now we have all the rect candidate, do a clustering here
    #print("NUmber of draw candidate is", len(draw_candidate))
    center_candidate = [[draw_candidate[0].cx, draw_candidate[0].width]]
    # print("center is ", center_candidate[0])
    rect_group = [[draw_candidate[0]]]
    for rect in draw_candidate[1:]:
        assigned = False
        for center in center_candidate:
            # print("candidate center is :", rect.cx)
            if abs(rect.cx - center[0]) < (rect.width + center[1])/2.0:
                rect_group[center_candidate.index(center)].append(rect)
                assigned = True
        # no center found create a new one
        if not assigned:
            center_candidate.append([rect.cx, rect.width])
            rect_group.append([rect])
    
    ## deal with each rect group: basically take their largest broader as the bbx
    final_rect = []
    # print("There are {} groups".format(len(rect_group)))
    for rects in rect_group:
        # get the four coordinate ready
        left = rects[0].cx - int(rects[0].width/2)
        right = rects[0].cx+int(rects[0].width/2)
        top = rects[0].cy-int(rects[0].height/2)
        bottom = rects[0].cy+int(rects[0].height/2)
        for rect in rects[1:]:
            if rect.cx + int(rect.width/2) > right:
                right = rect.cx + int(rect.width/2)
            if rect.cy + int(rect.height/2) > bottom:
                bottom = rect.cy + int(rect.height/2)
            if rect.cx - int(rect.width/2) < left:
                left = rect.cx - int(rect.width/2)
            if rect.cy - int(rect.height/2) < top:
                top = rect.cy - int(rect.height/2)
        # final_rect.append() TODO: Decide the output format
        # rect_cords = ((left, top), (left, bottom),
        #             (right, bottom), (right, top),
        #             (left, top))
        #TODO: Resize the bounding boxes from 384 1248 to 720 1280
        left = int((left - 283) * 720 / 384.0 - (right-left)*0.3)
        right = int((right - 283) * 720 / 384.0 + (right-left)*0.35)
        top = int(top * 720 / 384.0)
        bottom = int(bottom * 720 / 384.0)      
          
        final_rect.append([(left, top), (right, bottom)])
    
        # TODO: visualize 
        # draw.line(rect_cords, fill=(0,255,0), width=2)
